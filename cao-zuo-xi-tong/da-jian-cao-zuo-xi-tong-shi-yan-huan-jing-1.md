...
为了访问外网，这里还需要设置 /etc/sysctl.conf 文件中 net.ipv4.ip\_forward=1 参数，并且执行以下的命令，设置 NAT。
...
第一个要加的地方是 arch/x86/entry/syscalls/syscall\_64.tbl。这里面登记了所有的系统调用号以及相应的处理函数。
...
第三个就是对于这个系统调用的实现，方便起见，我们不再用 SYSCALL\_DEFINEx 系列的宏来定义了，直接在 kernel/sys.c 中实现。
